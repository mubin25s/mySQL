...
-- File 2: table&Insertvalues.sql (Data Population, Queries & Transactions)
...
-- ============================================================================
-- SECTION 1: DATA INSERTION (105 Total Records)
-- ============================================================================
...
-- Query 1: Basic Filtering (Cardiology doctors with fee < $200)
SELECT Doctor_ID, Doctor_Name, Specialization, Consultation_Fee, Room_No
FROM Doctors
WHERE Specialization = 'Cardiology' AND Consultation_Fee < 200.00;

-- Query 2: Pattern Matching (Name starts with 'A' or address contains 'Springfield')
SELECT Patient_ID, Full_Name, Gender, Phone, Address
FROM Patients
WHERE Full_Name LIKE 'A%' OR Address LIKE '%Springfield%';

-- Query 3: Date Range Filtering
SELECT Appointment_ID, Patient_ID, Doctor_ID, Appointment_Date, Time_Slot, Status
FROM Appointments
WHERE Appointment_Date BETWEEN '2026-08-01' AND '2026-08-05'
ORDER BY Appointment_Date ASC;

-- Query 4: Multi-table INNER JOIN
SELECT a.Appointment_ID, p.Full_Name AS Patient_Name, d.Doctor_Name, d.Specialization, a.Appointment_Date
FROM Appointments a
...
SELECT p.Patient_ID, p.Full_Name AS Patient_Name, a.Appointment_ID, r.Test_Name, r.Diagnosis
FROM Patients p
LEFT JOIN Appointments a ON p.Patient_ID = a.Patient_ID
LEFT JOIN Medical_Reports r ON a.Appointment_ID = r.Appointment_ID;

-- Query 6: 4-Way Join Financial Breakdown
SELECT pay.Payment_ID, p.Full_Name AS Patient_Name, d.Doctor_Name, pay.Amount, pay.Payment_Method
FROM Payments pay
INNER JOIN Appointments a ON pay.Appointment_ID = a.Appointment_ID
INNER JOIN Patients p ON a.Patient_ID = p.Patient_ID
INNER JOIN Doctors d ON a.Doctor_ID = d.Doctor_ID;

-- Query 7: Aggregation GROUP BY
SELECT Payment_Method, COUNT(Payment_ID) AS Total_Transactions, SUM(Amount) AS Total_Revenue
FROM Payments
GROUP BY Payment_Method;

-- Query 8: Aggregation GROUP BY and HAVING
SELECT d.Doctor_ID, d.Doctor_Name, COUNT(a.Appointment_ID) AS Completed_Count
FROM Doctors d
...
SELECT a.Appointment_ID, p.Full_Name AS Patient_Name, d.Doctor_Name, d.Consultation_Fee
FROM Appointments a
INNER JOIN Patients p ON a.Patient_ID = p.Patient_ID
INNER JOIN Doctors d ON a.Doctor_ID = d.Doctor_ID
WHERE d.Consultation_Fee > (SELECT AVG(Consultation_Fee) FROM Doctors);

-- Query 10: Non-Scalar Subquery
SELECT Patient_ID, Full_Name, Phone, Email
FROM Patients
WHERE Patient_ID IN (SELECT DISTINCT Patient_ID FROM Appointments WHERE Status = 'Cancelled');

-- Query 11: Complex Query (JOIN + WHERE + GROUP BY + HAVING)
SELECT d.Specialization, COUNT(DISTINCT a.Appointment_ID) AS Completed_Count, SUM(p.Amount) AS Total_Revenue
FROM Doctors d
INNER JOIN Appointments a ON d.Doctor_ID = a.Doctor_ID
...
-- "Top 3 Doctors by Revenue & Cancellation Rate"
-- ============================================================================
